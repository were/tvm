...
def compile_via_tvm(sym, arg_params, aux_params, symbol_file, data_shape):
    ...
    passes = list()
    passes.append(relay.qnn.transform.Legalize())
    passes.append(relay.qnn.transform.CanonicalizeOps())
    passes.append(relay.transform.Legalize())
    passes.append(relay.transform.ConvertLayout('NHWC'))
    ...
    with tvm.target.create(target):
        ...
        # mod = relay.transform.ConvertLayout('NHWC')(mod)
    ...
    with autotvm.apply_graph_best(graph_opt_sch_file):
        with relay.build_config(opt_level=3, disabled_pass=["AlterOpLayout"]):
            ...
            base = '/home/ubuntu/mxnet_compiled_models_vnni/tvm_' + symbol_file.split('/')[-1].replace('.json','')
